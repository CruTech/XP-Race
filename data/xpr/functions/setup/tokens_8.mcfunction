give @s minecraft:red_concrete{display:{Name:"{\"text\":\"Red Team\"}"}} 16
give @s minecraft:blue_concrete{display:{Name:"{\"text\":\"Blue Team\"}"}} 16

give @s minecraft:yellow_concrete{display:{Name:"{\"text\":\"Yellow Team\"}"}} 16
give @s minecraft:green_concrete{display:{Name:"{\"text\":\"Green Team\"}"}} 16

give @s minecraft:cyan_concrete{display:{Name:"{\"text\":\"Cyan Team\"}"}} 16
give @s minecraft:purple_concrete{display:{Name:"{\"text\":\"Purple Team\"}"}} 16
give @s minecraft:gray_concrete{display:{Name:"{\"text\":\"Gray Team\"}"}} 16
give @s minecraft:black_concrete{display:{Name:"{\"text\":\"Black Team\"}"}} 16

give @s minecraft:white_concrete{display:{Name:"{\"text\":\"Spectator\"}"}} 16

function xpr:setup/duration
